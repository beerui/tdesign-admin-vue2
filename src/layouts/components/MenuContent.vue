<template>
  <div>
    <template v-for="item in list">
      ...
      <t-submenu v-else :name="item.path" :value="item.path" :title="item.title" :key="item.name">
        <template #icon>
          <t-icon v-if="typeof item.icon === 'string' && item.icon" :name="item.icon" />
          <renderFnIcon :item="item" />
        </template>
  ...
  return list
    .map((item) => {
      ...
      return {
        ...
        icon: item.meta?.icon || '',
        children: getMenuList(item.children, item.path),
        meta: item.meta,
        type: item.type,
...
export default Vue.extend({
  ...
  methods: {
    ...
    clickHref(item) {
      console.log(item)
      if (item.type === 'Iframe') {
        this.$router.push({ path: '/i', query: item })
      }
    }
  },
});
